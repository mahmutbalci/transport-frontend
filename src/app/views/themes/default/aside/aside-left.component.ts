...
import { LayoutConfigService, MenuAsideService, MenuOptions, MenuConfigService } from '@core/_base/layout';
...
export class AsideLeftComponent implements OnInit, AfterViewInit {
	...
	isMenuItemIsActive(item): boolean {
		...
		// dashboard
		if (item.page !== '/' && this.baseComponent.currentRouteUrl.startsWith(item.page)) {
			return true;
		}
		return this.baseComponent.currentRouteUrl === item.page;
	}
	...
	selectLeftMenu(item) {
		if (item.page && item.page != null) {
			this.baseComponent.openTab(item.page).subscribe(sb => {
				if (sb) {
					this.router.navigateByUrl(item.page);
				}
			});
		} else if (item.aside && item.aside.items && item.aside.items.length > 0) {
			this.menuConfigService.setModel(item);
		}
	}

	getTranslateKey(item) {
		let key = item.title;
		if (item.translate && item.translate.length > 0) {
			key = item.translate;
		}
