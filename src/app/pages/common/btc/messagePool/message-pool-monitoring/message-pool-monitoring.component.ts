...
import { CustomerSearchRequestDto } from '@issuing/customer/customerSearchRequestDto.model';
...
export class MessagePoolMonitoringComponent implements OnInit {
	...
	@ViewChild("customerSearch") customerSearch: McustomerSearchComponent;
	...
	ngOnInit() {
		...
		this.issuingApi.getLookups(["MsnMessageTemplateTypeDef", "MsnMessageTemplateDef", "MsnMessageTypeDef", "CfgYesNoNumeric"]).then(res => {
	...
	loadDataSource() {
		if (this.messageMonitoringForm.value["endDate"] == null
			&& this.messageMonitoringForm.value["startDate"] == null
			&& this.customerSearch.customerSearchForm.value.customerNo == "") {
			const message = this.translate.instant('Issuing.Transaction.Exception.FillStartOrEndDateOrCustomerNo');
	...
	filterConfiguration(): any {
		let filter: any = {};
		filter.key = this.customerSearch.customerSearchForm.value.customerNo;
	...
	movedCustomer(customerSearchRequestDto: CustomerSearchRequestDto) {
		this.messageMonitoringForm.value["key"] = customerSearchRequestDto.customerNo;
		this.isOpenDialog = false;
		if (this.isGetClicked) {
			this.callGetService();
			this.isGetClicked = false;
		}
	}
